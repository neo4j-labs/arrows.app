...
describe('decideApply — webview → host write decision', () => {
  it('applies when text differs', () => {
    expect(decideApply({ currentText: 'old', nextText: 'new' })).toEqual({
      action: 'apply',
    });
  });

  it('drops when nextText === currentText (no-op)', () => {
    expect(decideApply({ currentText: 'same', nextText: 'same' })).toEqual({
      action: 'skip',
      reason: 'noop',
    });
  });
  ...
  it('rapid second edit still applies — no docVersion or in-flight check drops it', () => {
    ...
    expect(decideApply({ currentText: 'A', nextText: 'AB' })).toEqual({
      action: 'apply',
    });
  });
});
