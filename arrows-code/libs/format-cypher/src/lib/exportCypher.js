import {styleKeyToDatabaseKey, getNodeIdMap} from "@neo4j-arrows/model";
const plainIdentifier = new RegExp('^[A-Za-z][A-Za-z_]*$')

const escape = name => {
  const str = String(name ?? '')
  if (plainIdentifier.test(str)) {
    return str
  } else {
    // Escape any backticks inside the identifier by doubling them — Cypher's
    // identifier-quoting convention. Without this, a label/property key
    // containing a backtick could close the quoting and inject Cypher.
...
export const exportCypher = (graph, keyword, options) => {
  ...
  graph.relationships.forEach(relationship => {
    ...
    const growablePath = paths.find(path => matches(path[0]) || matches(path[path.length - 1]))
    if (growablePath) {
      ...
      paths.push([nodeIdMap[relationship.fromId], { relationship, direction: 'forward' }, nodeIdMap[relationship.toId]])
    }
  })
