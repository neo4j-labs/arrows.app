...
export const combineLabels = (nodes: Node[]) => {
  const allLabels = nodes
    ...
    .reduce(
      (accumulator, currentValue) => [
        ...new Set(accumulator.concat(currentValue)),
      ],
      []
    );
  ...
  allLabels.forEach((label) => {
    labels[label] = {
      status: nodes.every((node) => node.labels.includes(label))
        ? 'CONSISTENT'
        : 'PARTIAL',
    };
  });
    ...
    node.labels.forEach((label) => {
      if (label && !labelsInSelection.has(label)) {
        const existingLabel = labels.find(
          (labelEntry) => labelEntry.label === label
        );
        if (existingLabel) {
...
export const labelsFromDatabaseEntity = (entity: Node) => {
  return entity.labels
    .filter((label) => label !== 'Diagram0')
    .map(databaseLabelToLabel);
};
