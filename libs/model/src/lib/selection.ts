...
export const relationshipSelected = (selection:EntitySelection, relationshipId:Id) => {
  return selection.entities.some(entity =>
    entity.entityType === 'relationship' && entity.id === relationshipId
  )
}

export const relationshipEditing = (selection:EntitySelection, relationshipId:Id) => {
  return selection.editing &&
    selection.editing.entityType === 'relationship' && selection.editing.id === relationshipId
}

export const selectedRelationships = (graph:Graph, selection:EntitySelection) => {
