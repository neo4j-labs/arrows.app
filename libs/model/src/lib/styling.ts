...
const hasIcon = (node:Entity, style:StyleFunction) => !!style('node-icon-image') || !!style('relationship-icon-image')
const hasCaption = (node:Node) => node.caption && node.caption.length > 0
const hasLabels = (node:Node) => node.labels && node.labels.length > 0
const hasType = (relationship:Relationship) => relationship.type && relationship.type.length > 0
const hasProperty = (entity:Entity) => entity.properties && Object.keys(entity.properties).length > 0;
...
const styleFilters = {
  ...
  'NodeWithBorder': {
    relevantToNode: (node:Node, style:StyleFunction) => style('border-width') as number > 0
  },
...
const specificOrGeneral = (styleKey:string, entity:Entity, graphStyle:Style) => {
...
export const categoriesPresent = (nodes:Node[], relationships:Relationship[], graph:Graph) => {
