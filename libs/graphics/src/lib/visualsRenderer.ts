...
const layerManager = (() => {
  ...
  return {
    register: (name: string, drawFunction: DrawFn) =>
      layers.push({
        name,
        draw: drawFunction,
      }),
...
export const renderVisuals = ({
  ...
  ctx.clearRect(
    0,
    0,
    displayOptions.canvasSize.width,
    displayOptions.canvasSize.height
  );
  ...
  layerManager.register(
    'BACKGROUND IMAGE',
    backgroundImage.draw.bind(backgroundImage)
  );
  layerManager.register(
    'GUIDES ACTUAL POSITION',
    visualGuides.drawActualPosition.bind(visualGuides)
  );
  ...
  layerManager.register(
    'GUIDES SNAP LINES',
    visualGuides.draw.bind(visualGuides)
  );
