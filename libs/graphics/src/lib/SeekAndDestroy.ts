...
export class SeekAndDestroy {
  ...
  forwardToWaypoint(distance: number, turn: number, radius: number) {
    this.position = this.position.translate(
      new Vector(distance, 0).rotate(this.direction)
    );
  ...
  get endRelative() {
    return this.end
      .translate(this.position.vectorFromOrigin().invert())
    ...
    for (let i = 0; i < this.waypoints.length; i++) {
      ...
      if (vector1.distance() < d) {
        ...
        control = control.translate(
          vector2.scale(overlap / vector2.distance())
        );
      }
      if (vector2.distance() < d) {
        const overlap = d - vector2.distance();
        control = control.translate(
          vector1.scale(overlap / vector1.distance())
        );
      }
...
export const compareWaypoints = (a: Waypoint[], b: Waypoint[]): number => {
  ...
  if (Math.abs(aFirstWaypoint.distance - bFirstWaypoint.distance) > 0.0001) {
    return (
      Math.sin(a[0].turn) *
      Math.sign(bFirstWaypoint.distance - aFirstWaypoint.distance)
    );
  }
