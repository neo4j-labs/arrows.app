...
export class VisualRelationship {
  ...
  constructor(resolvedRelationship:ResolvedRelationship, graph:Graph, arrow:AnyArrow, editing:boolean, measureTextContext:TextMeasurementContext, imageCache:Record<string, ImageInfo>) {
    this.resolvedRelationship = resolvedRelationship
    this.arrow = arrow
    this.editing = editing

    const style = (styleAttribute:string) => getStyleSelector(resolvedRelationship.relationship, styleAttribute)(graph)
    ...
    switch (orientationName) {
      case 'horizontal':
        ...
        this.componentOffset = horizontalOffset(width, height, margin, alignment, arrow.shaftAngle())
        break
...
const horizontalOffset = (width:number, height:number, margin:number, alignment:TextOrientation, shaftAngle:number) => {
...
const parallelOffset = (height:number, margin:number, positionName:'above'|'inline'|'below') => {
...
const perpendicularOffset = (height:number, margin:number, alignment:TextOrientation) => {
