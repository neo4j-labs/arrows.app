import {Coordinate, EntitySelection, Point, ViewTransformation} from "@neo4j-arrows/model";
...
export class TransformationHandles {
  handles: { corner: { x: string; y: string; }; anchor: Point; topLeft: Point; }[] = []
  color = 'blue' // ABK what is a sensible default color?
  constructor(visualGraph:VisualGraph, selection:EntitySelection, mouse:any, viewTransformation:ViewTransformation) {
    ...
    if (mouse.dragType === 'NONE' && nodeIds.length > 1) {
      ...
      const transform = (position:Point) => viewTransformation.transform(position)
