...
export const computeRelationshipAttachments = (graph:Graph, visualNodes:Record<string, VisualNode>) => {
  const nodeAttachments:Record<string, VisualAttachment> = {}
  const countAttachment = (nodeId:string, attachmentOptionName:keyof VisualAttachment) => {
    const nodeCounters = nodeAttachments[nodeId] || (nodeAttachments[nodeId] = {} as VisualAttachment)
    nodeCounters[attachmentOptionName] = (nodeCounters[attachmentOptionName] || 0) + 1
  }

  graph.relationships.forEach(relationship => {
    const style = (styleAttribute:string) => getStyleSelector(relationship, styleAttribute)(graph)
    countAttachment(relationship.fromId, style('attachment-start'))
    countAttachment(relationship.toId, style('attachment-end'))
  })

  const centralAttachment = (nodeId:string, attachmentOptionName:keyof VisualAttachment):VisualAttachment => {
...
const findOption = (optionName:string) => {
  return attachmentOptions.find(option => option.name === optionName) || { name: 'normal', angle:0 } // ABK: `angle: 0` is dubious, perhaps `angle` should be optional?
}

const computeRadiusOrdinal = (path:{polarity:number}, ordinal:number, total:number) => {
  if (path) {
