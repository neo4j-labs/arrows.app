import {Coordinate, getStyleSelector, Graph, Node, Point} from "@neo4j-arrows/model";

export const calculateBoundingBox = (nodes:Node[], graph:Graph, scale = 1) => {
  if (nodes.length === 0) {
    return null
  }

  const getPosition = (node:Node) => node.position.scale(scale)
  const radius = (node:Node) => getStyleSelector(node, 'radius')(graph) * scale
...
export const getDistanceToLine = (x1:number, y1:number, x2:number, y2:number, x3:number, y3:number) => {
  const px = x2 - x1
  const py = y2 - y1
