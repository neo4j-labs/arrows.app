...
export const fitTextToCircle = (
  ...
  const linesForStaringPoint = (
    ...
    while (words.length > 0) {
      if (
        measureWidth(currentLineWithExtraWord(words[0])) >
        currentLine.extent.width
      ) {
    ...
    return {
      ...
      emptyLinesBelow:
        availableLines -
        (lines.length > 0 ? lines[lines.length - 1].index : startingLine) -
        1,
      wordsRemaining: words.length,
    };
  };

  const possibleLayoutsA = range(Math.ceil(maxLines / 2)).map((startingLine) =>
    linesForStaringPoint(maxLines, startingLine)
  );
  const possibleLayoutsB = range(Math.ceil((maxLines - 1) / 2)).map(
    (startingLine) => linesForStaringPoint(maxLines - 1, startingLine)
  );
  ...
  const balancedLayout: TextLayout = (() => {
    const mostWords = filterLowest(
      allPossibleLayouts,
      (layout) => layout.wordsRemaining
    );
    const fewestLines = filterLowest(
      mostWords,
      (layout) => layout.lines.length
    );
    const mostBalanced = filterLowest(fewestLines, (layout) =>
      Math.abs(layout.startingLine - layout.emptyLinesBelow)
    );
  ...
  const biggestGap = (layout: TextLayout) =>
    Math.max(...layout.lines.map(spacePerLine));

  const layoutWithSwappedLines = (
    layout: TextLayout
