...
export interface DrawingContext {
  ...
  rect(x:number, y:number, width:number, height:number, r:number, fill:boolean, stroke:boolean):void

  image(imageInfo:ImageInfo, x:number, y:number, width:number, height:number):void

  imageInCircle(imageInfo:ImageInfo, cx:number, cy:number, radius:number):void
