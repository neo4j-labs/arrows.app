          ...
          for (let i = 0; i < styleElement.sheet.cssRules.length; i++) {
            ...
            if (isCSSStyleRule(cssRule)) {
              const fontCodePoints = UnicodeRange.parse(
                cssRule.style.getPropertyValue('unicode-range').split(', ')
              );
              const fontNeeded = Array.from(graphCodePoints).some((codePoint) =>
...
const googleFontCss = (fontFamily: string) =>
