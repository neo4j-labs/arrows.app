...
export const renderPngAtScaleFactor = (graph:Graph, cachedImages:Record<string,ImageInfo>, scaleFactor:number, maxPixels:number, transparentBackground:boolean) => {
  const renderState = {
    graph,
    cachedImages,
    selection: {
...
export const renderPngForThumbnail = (graph:Graph, cachedImages:Record<string,ImageInfo>) => {
  const renderState = {
    graph,
    cachedImages,
    selection: {
