...
export const dataUrlToString = (d:DataURL | undefined) =>
  (d === undefined || d === null) ? `Missing DataURL`
    : (typeof d === 'string') ? d
    : new TextDecoder('utf-8').decode(d) // ABK: a bit of a hack

export const containsCachedImage = (cachedImages:Record<string,ImageInfo>, imageUrl:string):boolean => {
...
export const getCachedImage = (cachedImages:Record<string,ImageInfo>, imageUrl:string):ImageInfo => {
