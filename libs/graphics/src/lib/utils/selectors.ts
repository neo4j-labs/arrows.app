...
import {Node, Graph, nodeEditing, nodeSelected, relationshipSelected, selectedNodeIds, EntitySelection, Point} from "@neo4j-arrows/model";
...
const getViewTransformation = (state:{viewTransformation:any}) => state.viewTransformation
const getCachedImages = (state:{cachedImages:Record<string, ImageInfo>}) => state.cachedImages

export const getPresentGraph = (state:{graph:any}) => state.graph.present || state.graph
...
export const getVisualNode = (() => {
  const factory = (node:Node, graph:Graph, selection:EntitySelection, cachedImages:Record<string, ImageInfo>) => {
