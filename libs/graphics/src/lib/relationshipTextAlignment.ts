...
export const readableAngle = (orientation:RelationshipTextOrientation, shaftAngle:number) => {
...
const angleForOrientation = (orientation:RelationshipTextOrientation, shaftAngle:number) => {
  switch (orientation) {
    case 'parallel':
      return shaftAngle
    case 'perpendicular':
...
export const alignmentForShaftAngle = (orientation:RelationshipTextOrientation, position:string, shaftAngle:number):TextOrientation => {
  if (position === 'inline') {
    return {
      horizontal: 'center',
      vertical: 'center'
