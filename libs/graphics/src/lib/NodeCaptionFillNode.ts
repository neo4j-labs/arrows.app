...
export class NodeCaptionFillNode {
  ...
  constructor(caption:string, radius:number, editing:boolean, style:StyleFunction, textMeasurement:TextMeasurementContext) {
  ...
  draw(ctx:DrawingContext) {
    ...
    for (let i = 0; i< this.layout.lines.length; i++) {
      ...
      drawTextLine(ctx, this.layout.lines[i] as string, new Point(0, yPos), 'center')
    }
