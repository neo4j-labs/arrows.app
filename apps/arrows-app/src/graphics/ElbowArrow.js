...
export class ElbowArrow {
  constructor(
    ...
    const fixedEnd =
      startAttachment && startAttachment.attachment.name !== 'normal'
        ? 'start'
        : 'end';
    const fixedAttachment =
      fixedEnd === 'start' ? startAttachment : endAttachment;
    ...
    const fixedDivergeRadius =
      fixedEnd === 'start'
        ? startRadius + startAttachment.minNormalDistance
        : endRadius +
          Math.max(
    ...
    const offset =
      (fixedAttachment.ordinal - (fixedAttachment.total - 1) / 2) * 10;
    const fixedAttach = fixedCentre.translate(
      new Vector(fixedRadius, offset).rotate(fixedAttachAngle)
    );
    const fixedDiverge = fixedCentre.translate(
      new Vector(fixedDivergeRadius, offset).rotate(fixedAttachAngle)
    );
    const normalCentreRelative = normalCentre
      .translate(fixedDiverge.vectorFromOrigin().invert())
      .rotate(-fixedAttachAngle);
    const arcCentre = new Point(
      0,
      normalCentreRelative.y < 0 ? -arcRadius : arcRadius
    );
    ...
    const control = fixedAttach.translate(
      new Vector(d, 0).rotate(fixedAttachAngle)
    );
    const normalAttachAngle = control.vectorFrom(normalCentre).angle();
    const normalAttach = normalCentre.translate(
      new Vector(normalRadius, 0).rotate(normalAttachAngle)
    );
    ...
    const path = new SeekAndDestroy(
      ...
      fixedAttachAngle,
      normalAttach,
      normaliseAngle(normalAttachAngle + Math.PI)
    );
    path.forwardToWaypoint(
      d + fixedDivergeRadius - fixedRadius,
      Math.sign(path.endDirectionRelative) * theta,
      arcRadius
    );
