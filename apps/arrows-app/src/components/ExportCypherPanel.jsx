...
class ExportCypherPanel extends Component {
  ...
  runInNeo4jBrowser = (cypher) => {
    const url =
      'neo4j-desktop://graphapps/neo4j-browser?cmd=edit&arg=' +
  ...
  render() {
    ...
    const mergeWarning =
      this.state.keyword === 'MERGE' ? (
        <Message info>
          MERGE query behaviour depends on what data is already present in the
          database. You may need to edit the query to achieve exactly the
          behaviour you are looking for. Please see{' '}
          <a
    ...
    return (
      <Form>
        ...
        <Form.Field>
          <Button
            onClick={() => this.copyToClipboard(cypher)}
            primary
            icon
            labelPosition="left"
          >
          ...
          <Button
            onClick={() => this.runInNeo4jBrowser(cypher)}
            primary
            icon
            labelPosition="left"
          >
