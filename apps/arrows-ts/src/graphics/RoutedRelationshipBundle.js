  ...
  constructor(relationships, graph, selection, measureTextContext, imageCache) {
    ...
    const leftRadius = Math.max(
      ...arrowDimensions.map((arrow) =>
        arrow.leftToRight ? arrow.startRadius : arrow.endRadius
      )
    );
    const rightRadius = Math.max(
      ...arrowDimensions.map((arrow) =>
        arrow.leftToRight ? arrow.endRadius : arrow.startRadius
      )
    );
    ...
    if (relationships[0].startAttachment || relationships[0].endAttachment) {
      ...
      const neighbourAngles = neighbourPositions(selfNode, graph).map(
