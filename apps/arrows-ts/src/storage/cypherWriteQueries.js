...
export const writeQueriesForAction = (action, graph) => {
  switch (action.type) {
    case 'CREATE_NODE': {
      ...
      return (session) =>
        session.run(
          'CREATE (n:Diagram0 {_id: $id, _x: $x, _y: $y, ' +
    ...
    case 'CREATE_NODE_AND_RELATIONSHIP': {
      ...
      return (session) =>
        session.run(
          'MATCH (n:Diagram0 {_id: $sourceNodeId}) ' +
    ...
    case 'CONNECT_NODES': {
      return (session) =>
        session.run(
          'MATCH (source:Diagram0 {_id: $sourceNodeId}), (target:Diagram0 {_id: $targetNodeId}) ' +
    ...
    case 'SET_NODE_CAPTION': {
      return (session) =>
        session.run(
          'MATCH (n:Diagram0) WHERE n._id IN $ids ' +
    ...
    case 'ADD_LABEL': {
      return (session) =>
        session.run(
          'MATCH (n:Diagram0) WHERE n._id IN $ids ' +
            'SET n:`' +
            labelToDatabaseLabel(action.label) +
            '`',
          {
            ids: selectedNodeIds(action.selection),
          }
    ...
    case 'RENAME_LABEL': {
      return (session) =>
        session.run(
          'MATCH (n:Diagram0) WHERE n._id IN $ids ' +
            'SET n:`' +
            labelToDatabaseLabel(action.newLabel) +
            '`' +
            'REMOVE n:`' +
            labelToDatabaseLabel(action.oldLabel) +
            '`',
          {
            ids: selectedNodeIds(action.selection),
          }
    ...
    case 'RENAME_PROPERTY': {
      return (session) => {
        session.run(
          'MATCH (n:Diagram0) WHERE n._id IN $ids ' +
            'SET n.`' +
            propertyKeyToDatabaseKey(action.newPropertyKey) +
            '` = n.`' +
            propertyKeyToDatabaseKey(action.oldPropertyKey) +
            '` ' +
            'REMOVE n.`' +
            propertyKeyToDatabaseKey(action.oldPropertyKey) +
            '`',
        ...
        return session.run(
          'MATCH (:Diagram0)-[r]->(:Diagram0) WHERE r._id IN $ids ' +
            'SET r.`' +
            propertyKeyToDatabaseKey(action.newPropertyKey) +
            '` = r.`' +
            propertyKeyToDatabaseKey(action.oldPropertyKey) +
    ...
    case 'SET_PROPERTY': {
      ...
      return (session) => {
        session.run(
          'MATCH (n:Diagram0) WHERE n._id IN $ids ' + 'SET n += $properties',
          {
            ids: selectedNodeIds(action.selection),
    ...
    case 'SET_ARROWS_PROPERTY': {
      ...
      if (relationshipStyleAttributes.includes(action.key)) {
        queries.push({
          cypher:
            'MATCH (:Diagram0)-[r]->(:Diagram0) WHERE r._id IN $ids SET r += $properties',
          params: {
            ids: selectedRelationshipIds(action.selection),
            properties: styleProperties,
          },
      ...
      return (session) => {
        const promises = queries.map((query) =>
          session.run(query.cypher, query.params)
        );
    ...
    case 'REMOVE_ARROWS_PROPERTY': {
      return (session) => {
        session.run(
          'MATCH (n:Diagram0) WHERE n._id IN $ids ' +
            'REMOVE n.`' +
            styleKeyToDatabaseKey(action.key) +
            '`',
          {
            ids: selectedNodeIds(action.selection),
          }
    ...
    case 'REMOVE_PROPERTY': {
      return (session) => {
        session.run(
          'MATCH (n:Diagram0) WHERE n._id IN $ids ' +
            'REMOVE n.`' +
            propertyKeyToDatabaseKey(action.key) +
            '`',
          {
            ids: selectedNodeIds(action.selection),
          }
      ...
      if (action.nodePositions.length !== 0) {
        return (session) => {
          ...
          action.nodePositions.forEach(({ nodeId, position }) => {
            result = session.run(
              'MATCH (n:Diagram0 {_id: $id}) ' + 'SET n._x = $x, n._y = $y',
              {
                id: nodeId,
                x: position.x,
                y: position.y,
    ...
    case 'DUPLICATE_NODES_AND_RELATIONSHIPS': {
      return (session) => {
        ...
        Object.keys(action.nodeIdMap).forEach((newNodeId) => {
          ...
          result = session.run(
            `MATCH (old:Diagram0 {_id: $oldNodeId})
            WITH old
            CREATE (new:Diagram0)
            SET new = old
