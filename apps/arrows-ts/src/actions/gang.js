...
export const createClusterGang =
  (nodePositions, initialPositions) => (dispatch, getState) => {
    const state = getState();
    const graph = getPresentGraph(state);
    const combinedNodeIds = nodePositions.map((nodePos) => nodePos.nodeId);
    const combinedNodes = graph.nodes.filter((node) =>
      combinedNodeIds.includes(node.id)
    );
...
export const createCluster = (
  ...
  nodeType,
  initialPositions,
  initialClusterPosition = position
) => ({
  type: 'CREATE_CLUSTER',
  category: 'GRAPH',
  nodeId,
...
export const removeClusterGang = (nodeId) => (dispatch, getState) => {
  ...
  if (!cluster.position.isEqual(cluster.initialPosition)) {
    const newMemberPositions = cluster.members.map((nodePos) => ({
      ...
      position: nodePos.position.translate(
        cluster.position.vectorFrom(cluster.initialPosition)
      ),
    }));
    ...
    case 'NODE_RING':
      ...
      if (dragToCreate.sourceNodeId) {
        const sourceNode = graph.nodes.find((node) =>
          idsMatch(node.id, dragToCreate.sourceNodeId)
        );
    ...
    case 'HANDLE':
      const shouldCombine = (positions) => {
        ...
        for (let i = 1; i < positions.length - 1; i++) {
          if (
            positions[i].position.x === position.x &&
            positions[i].position.y === position.y
          ) {
            result = true;
          } else {
            result = false;
